...
interface DashboardClientProps {
  summaryData: Summary;
  dailyCasualtiesData: DailyCasualties[];
  infrastructureData: InfrastructureDamaged[];
}

function InfrastructureTable({ data }: { data: InfrastructureDamaged[] }) {
  return (
          ...
          {data.map((item) => (
            <TableRow key={item.type}>
              ...
              <TableCell className="text-right text-destructive">
                {item.destroyed?.toLocaleString() ?? "-"}
              </TableCell>
              <TableCell className="text-right">
                {item.damaged && item.damaged > 0 ? item.damaged.toLocaleString() : "-"}
              </TableCell>
            </TableRow>
          ))}
        </TableBody>
...
export function DashboardClient({ summaryData, dailyCasualtiesData, infrastructureData }: DashboardClientProps) {
  const gazaSummary = summaryData || {};
  ...
  const homesDestroyed = infrastructureData.find(item => item.type === "Housing Units")?.destroyed || 0;
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
         ...
         <Card>
            <CardHeader>
                ...
                <CardDescription>
                Key civilian infrastructure destroyed or damaged.
                </CardDescription>
            </CardHeader>
            <CardContent>
