...
export default async function PalestineDataHub() {
  ...
  try {
    const [
      ...
      supabase.from('gaza_daily_casualties').select('killed_cum, injured_cum, killed_children_cum, killed_women_cum').not('killed_children_cum', 'is', null).not('killed_women_cum', 'is', null).order('date', { ascending: false }).limit(1).single(),
    ...
    return (
      <main className="bg-[#111] text-white p-4 md:p-8 space-y-16">
        ...
        {/* This component gets the data from the queries above */}
        <Overview gazaData={gazaResult.data} westBankData={westBankResult.data} />
        
        {/* This component gets the 'timelineResult.data' */}
